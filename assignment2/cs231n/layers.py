...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        mb_var = np.sum((x-mb_mean)**2,axis=0)/float(N)
        x1=(x-mb_mean)/np.sqrt(mb_var+eps)
        out = x1*gamma+beta
...
def batchnorm_backward(dout, cache):
    ...
    d1=np.sum(dout*gamma*(x-mb_var),axis=0)    
    d2=dout*gamma*np.sqrt(mb_var+eps)
    d3 = d1*(-1./(mb_var+eps))
    d4 = d3*(1./(2*np.sqrt(mb_var+eps)))
    d5 = d4*(1./N)*np.ones((N,D))
    d6 = d5*2*(x-mb_mean)
    dx=d2+d6
    dmu=-1*np.sum(d2+d6,axis=0)
    d8=(1./N)*np.ones((N,D))*dmu
    dx+=d8"""
    dbeta = np.sum(dout, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    N, D = dout.shape
    x_hat,gamma,x_mu,inv_var,sqrtvar,var,eps = cache  
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(F):
            ...
            for k in range(H1):
                for z in range(W1):                                        
                    dw[j]+=x_pad[i,:,k*stride:k*stride+HH,z*stride:z*stride+WW]*dout[i,j,k,z]
                    dx[i,:,k*stride:k*stride+HH,z*stride:z*stride+WW]+=w[j]*dout[i,j,k,z]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    dx=dx[:,:,1:-1,1:-1]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for j in range(C):            
            for k in range(H1):
                for z in range(W1):    
                    out[i][j][k][z]=np.max(x[i,j,k*stride:k*stride+pool_height,z*stride:z*stride+pool_width])                                    
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):            
            for k in range(H1):
                for z in range(W1):    
                    ind = np.argmax(x[i, j, k*stride:k*stride+HH, z*stride:z*stride+WW])
                    ind1, ind2 = np.unravel_index(ind, (HH, WW))
                    dx[i, j, k*stride:k*stride+HH, z*stride:z*stride+WW][ind1, ind2] = dout[i, j, k, z]                    
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
