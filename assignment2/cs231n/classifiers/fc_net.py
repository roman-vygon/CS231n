...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.normalization=='layernorm':
            self.bn_params = [{} for i in range(0,self.num_layers - 1)]
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, num_layers):       
            ...
            layer, n_cache_layer = affine_n_relu_forward(layer,
                                                     self.params['W%d' % i],
                                                     self.params['b%d' % i],
                                                     gamma,
                                                     beta,                                                     
            ...
            cache_layer.append(n_cache_layer)
            if self.use_dropout:
                layer, d_cache = dropout_forward(layer,self.dropout_param)
                cache_layer.append(d_cache)                
        
        WLast = 'W%d' % self.num_layers
        bLast = 'b%d' % self.num_layers
        scores, cache_scores = affine_forward(layer,
                                          self.params[WLast],
                                          self.params[bLast])       
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        for i in range(num_layers-1, 0, -1):          
          if self.use_dropout:
            dx=dropout_backward(dx,cache_layer.pop())        
          dx,grads['W'+str(i)],grads['b'+str(i)],grads['gamma'+str(i)],grads['beta'+str(i)]=affine_n_relu_backward(dx,cache_layer.pop(),self.normalization)          
          if (self.normalization == None):            
